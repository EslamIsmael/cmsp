...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.prospect.title_singular') }}
    </div>
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.prospects.update", [$prospect->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                @endif
                <span class="help-block">{{ trans('cruds.prospect.fields.logo_helper') }}</span>
            </div>
            <div class="form-group">
                <label for="files">{{ trans('cruds.prospect.fields.files') }}</label>
                <div class="needsclick dropzone {{ $errors->has('files') ? 'is-invalid' : '' }}" id="files-dropzone">
                </div>
                @if($errors->has('files'))
                    <span class="text-danger">{{ $errors->first('files') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.prospect.fields.files_helper') }}</span>
            </div>
            <div class="form-group">
...
<script>
    ...
    url: '{{ route('admin.prospects.storeMedia') }}',
    maxFilesize: 5, // MB
    ...
    init: function () {
@if(isset($prospect) && $prospect->logo)
      var file = {!! json_encode($prospect->logo) !!}
          this.options.addedfile.call(this, file)
      this.options.thumbnail.call(this, file, '{{ $prospect->logo->getUrl('thumb') }}')
...
Dropzone.options.filesDropzone = {
    ...
    init: function () {
@if(isset($prospect) && $prospect->files)
          var files =
            {!! json_encode($prospect->files) !!}
              for (var i in files) {
              var file = files[i]
              this.options.addedfile.call(this, file)
              file.previewElement.classList.add('dz-complete')
